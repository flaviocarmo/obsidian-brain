...
2. **Onde** (pasta canônica em `wiki/`, taxonomia derivada do dado): sessão/snapshot datado em `journal/` (nome `Sessao YYYY-MM-DD <tema>.md`), conhecimento técnico em `domains/<subdomínio>/` (ex.: platforms, data, infra, geo, agents, method, business — use os que o vault já tem), contrato/cliente/pendência viva em `contracts/`, pessoa em `people/`. ANTES de criar, busque no basic-memory se já existe página para atualizar. Update vence create.
